...
// Detect what kind of window this is: 'normal' | 'incognito' | 'tor'
const WINDOW_MODE = new URLSearchParams(window.location.search).get('windowMode') || 'normal';
...
async function init() {
  ...
  wireDownloads();
  if (WINDOW_MODE === 'incognito') createTab('', false, true);
  else if (WINDOW_MODE === 'tor')  createTab('', true, false);
  else                             createTab();
}
...
function wireControls() {
  ...
  // + button in tab bar and "New Tab" in menu:
  // In normal window  → new normal tab
  // In incognito window → new incognito tab
  // In tor window     → new tor tab
  $('new-tab-btn').onclick = () => {
    if (WINDOW_MODE === 'incognito') createTab('', false, true);
    else if (WINDOW_MODE === 'tor')  createTab('', true, false);
    else                             createTab();
  ...
  // Incognito button: open new incognito window (from normal), or new incognito tab (already in incognito)
  $('btn-new-incognito').onclick = () => {
    if (WINDOW_MODE === 'normal') window.electronAPI.openIncognitoWindow();
    else createTab('', false, true);
  };

  // Tor button: open new tor window (from normal/incognito), or new tor tab (already in tor window)
  $('btn-new-tor').onclick = () => {
    if (WINDOW_MODE === 'tor') createTab('', true, false);
    else window.electronAPI.openTorWindow();
  };
  ...
  $('btn-print').onclick = async () => {
    ...
    if (!wv) return;
    try {
      // webview.print() is the correct Electron API
      wv.print();
    } catch (e) {
      // fallback
      wv.executeJavaScript('window.print()').catch(() => {});
    }
  };
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key==='N') { e.preventDefault();
      if (WINDOW_MODE === 'incognito') createTab('', false, true);
      else window.electronAPI.openIncognitoWindow();
    }
...
function wireDownloads() {
  window.electronAPI.onDownloadStarted(d => {
    // Add a pending entry immediately so the user sees it start
    downloads.unshift({
      id:       Date.now(),
      filename: d.filename,
      url:      d.url,
    ...
    renderDownloads();
    setStatus(`Downloading: ${d.filename}`);
  });

  window.electronAPI.onDownloadComplete(d => {
    // Update the matching pending entry to completed
    const existing = downloads.find(dl => dl.filename === d.filename && dl.status === 'downloading');
    if (existing) {
    ...
    renderDownloads();
    setStatus(`Downloaded: ${d.filename}`);
  });

  window.electronAPI.onDownloadFailed(d => {
    const existing = downloads.find(dl => dl.filename === d.filename && dl.status === 'downloading');
    if (existing) {
    ...
    renderDownloads();
    setStatus(`Download failed: ${d.filename}`);
  });
}
...
function renderDownloads() {
  ...
  downloads.forEach(d => {
    ...
    // Status icon
    const icon = E('div', 'dl-icon');
    if (d.status === 'completed') {
      icon.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        <circle cx="7" cy="7" r="6" stroke="var(--grn)" stroke-width="1.4"/>
        <path d="M4 7l2 2 4-4" stroke="var(--grn)" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>`;
    } else if (d.status === 'failed') {
      icon.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        <circle cx="7" cy="7" r="6" stroke="var(--err,#f87171)" stroke-width="1.4"/>
        <path d="M5 5l4 4M9 5l-4 4" stroke="var(--err,#f87171)" stroke-width="1.4" stroke-linecap="round"/>
      </svg>`;
    } else {
      icon.innerHTML = `<div class="spinner" style="width:14px;height:14px;border-width:1.5px"></div>`;
    }
    icon.style.cssText = 'flex-shrink:0;display:flex;align-items:center;justify-content:center;width:22px;';

    const info = E('div', 'list-item-info');
    const t    = E('div', 'list-item-title');
    t.textContent = d.filename || 'Download';

    const u = E('div', 'list-item-url');
    if (d.status === 'completed' && d.savePath) {
      u.textContent = d.savePath;
    } else if (d.status === 'failed') {
      u.textContent = 'Download failed';
      u.style.color = 'var(--err,#f87171)';
    } else {
      u.textContent = 'Downloading…';
      u.style.color = 'var(--warn,#fb923c)';
    }

    const time = E('div', 'list-item-time');
    time.textContent = new Date(d.date).toLocaleTimeString('en-US', { hour:'2-digit', minute:'2-digit' });

    info.append(t, u);

    const del = E('button', 'list-del');
    del.textContent = '✕';
    del.title = 'Remove from list';
    del.onclick = () => {
      const i = downloads.indexOf(d);
      if (i >= 0) downloads.splice(i, 1);
      renderDownloads();
    };

    el.append(icon, info, time, del);
    list.appendChild(el);
  });
}
...
function createTab(url='', isPrivate=false, isIncognito=false) {
  const id = ++tabCounter;
  const tab = {
  ...
  // Private tabs:   non-persistent, routed through Tor
  // Incognito tabs: non-persistent, normal network (no Tor)
  // Regular tabs:   persistent session
  let partition;
  if (isPrivate)        partition = `tor:tab${id}`;
  else if (isIncognito) partition = `incognito:tab${id}`;
  else                  partition = `persist:tab${id}`;
  wv.setAttribute('partition', partition);
  wv.setAttribute('allowpopups', '');
  // Force light color-scheme so the browser's dark/custom theme never
  // bleeds into website prefers-color-scheme media queries.
  wv.setAttribute('webpreferences','contextIsolation=yes,colorScheme=light');

  // For Tor tabs: spoof UA on the webview element to match what main.js sends in headers
  if (isPrivate) {
    wv.setAttribute('useragent', 'Mozilla/5.0 (Windows NT 10.0; rv:109.0) Gecko/20100101 Firefox/115.0');
  }
  tab.webview = wv;
  $('webview-container').appendChild(wv);

  // Wait for the proxy to be applied before navigating, so private tabs
  // always go through Tor from the very first request.
  window.electronAPI.hookPartition(partition).then(() => {
    wireWV(wv, id);
    renderTabs();
    activateTab(id);
    if (url) {
...
function renderTabs() {
  ...
  tabs.forEach((tab, tabIdx) => {
    const el = E('div', 'tab' + (tab.id===activeTabId?' active':'') + (tab.category?' has-cat':'') + (tab.private?' private-tab':'') + (tab.incognito?' incognito-tab':''));
    ...
    title.textContent = (tab.private ? '🧅 ' : '') + (tab.title || 'New Tab');
...
const CAT_ICONS = {
  ...
  travel:'', shopping:'', social_media:'', legal:'',
...
function wireWV(wv, tabId) {
  ...
  wv.addEventListener('did-finish-load', () => {
    const t=getTab(tabId); if(!t)return;
    if(t.url&&!t.url.startsWith('about:') && !t.private && !t.incognito) addHistory(t.url, t.title);
    // Inject anti-fingerprint spoofing for Tor tabs via executeJavaScript (main world)
    // This replaces the old setPreloads approach which conflicted with contextIsolation=yes
    if (t.private) {
      window.electronAPI.getTorPreloadScript().then(script => {
        if (script) wv.executeJavaScript(script).catch(() => {});
      });
    }
...
function showNewTab(show){
  const tab = getTab(activeTabId);
  $('newtab-page').classList.remove('newtab-visible');
  $('incognito-page').classList.remove('private-visible');
  $('tor-page').classList.remove('private-visible');
  if (!show) return;
  if (tab?.private)        $('tor-page').classList.add('private-visible');
  else if (tab?.incognito) $('incognito-page').classList.add('private-visible');
  else {
    $('newtab-page').classList.add('newtab-visible');
    setTimeout(() => $('newtab-search-input').focus(), 80);
  }
}
